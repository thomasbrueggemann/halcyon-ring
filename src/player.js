...
import {
  RF, RMAJ, RT, CHORD_DROP, EYE_HEIGHT, WALK_SPEED, RUN_SPEED, JUMP_SPEED,
  THRUST_ACCEL, MOUSE_SENS, G_FULL,
} from './config.js';
import { torusPosition, frameQuaternion, upAt, tangentAt, worldToTorus } from './torusMath.js';
...
export class Player {
  ...
  update(dt, gravityScale) {
    ...
    if (this.enabled && zeroG) {
      ...
      const lookQ = _q.clone().multiply(_qPitch);
      _wish.set(0, 0, 0);
      _tmp.set(0, 0, -1).applyQuaternion(lookQ);
      _wish.addScaledVector(_tmp, iz);
      _wish.addScaledVector(_right, ix);
      if (this.key('Space') && !this.suppressSpace) _wish.addScaledVector(_up, 1);
      if (this.key('KeyC') || this.key('ControlLeft')) _wish.addScaledVector(_up, -1);
      if (_wish.lengthSq() > 0) {
        _wish.normalize();
      ...
      _wish.addScaledVector(_up, -_wish.dot(_up));
      ...
      _tmp.copy(this.vel).addScaledVector(_up, -vUp);   // horizontal
      const accel = this.grounded ? 14 : 3;
      _tmp.lerp(_wish, Math.min(1, accel * dt));
      this.vel.copy(_tmp).addScaledVector(_up, vUp);
      this.speedAlongGround = _tmp.length();

      if (this.grounded && this.key('Space') && !this.suppressSpace && gravityScale > 0.25) {
        this.vel.addScaledVector(_up, JUMP_SPEED * Math.sqrt(gravityScale));
        this.grounded = false;
      }
    }

    // gravity (scaled by wheel spin)
    this.vel.addScaledVector(_up, -G_FULL * gravityScale * dt);
    ...
    if (this.h <= 0) {
      this.h = 0;
      const vUp = this.vel.dot(_up);
      if (vUp < 0) this.vel.addScaledVector(_up, -vUp * (zeroG ? 1.4 : 1)); // soft bounce in zero-g
    ...
    // ── hull cross-section constraint (walls + glass ceiling) ──
    {
      ...
      if (dist > maxR) {
        ...
        _tmp.copy(_up).multiplyScalar(-nx);
        _tmp.y += ny;
    ...
    if (push) {
      ...
      if (len > 1e-6) {
        _tmp.copy(_tan).multiplyScalar(push.ds / len);
    ...
    _qYaw.setFromAxisAngle(Y_AXIS, this.yaw);
    _qPitch.setFromAxisAngle(X_AXIS, this.pitch);
    _q.multiply(_qYaw).multiply(_qPitch);
  ...
  driftKick(rng) {
    tangentAt(this.theta, _tan);
    upAt(this.theta, _up);
    this.vel.addScaledVector(_tan, 2.5 + rng() * 2.5);
    this.vel.addScaledVector(_up, 1.2 + rng() * 1.5);
